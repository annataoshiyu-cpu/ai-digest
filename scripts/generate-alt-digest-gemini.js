...
# 单条目撰写模板
每条目先用一行写交易标题，机构名和标的名称用"**"包裹以标记加粗；标题下方用"• "开头的项目符号，逐行列出各字段，不要写成一段连续的长段落：

**[主体机构]** 收购/投资/募集 **[标的名称]**
• 标的概况：[业务]，所处行业为[行业]，商业模式为[一句话描述]（[上市/非上市状态]，[交易所：代码] 或 "非上市公司"）
• 估值：[金额]（参照基准：[YYYY年MM月DD日]的[市场收盘价/项目评估价值/融资后估值等]；估值倍数：[类型及数值，如无则写"暂无公开倍数数据"]）
• 摘要：[2句话概括交易内容与目的]
• 关键数据：[交易规模、募资方式、投资比例等]
• 解读：[1-2句实质性判断——为何重要、对市场/赛道意味着什么]
• 来源：[媒体名称]
...
- 保持客观中立，不给出投资建议或买卖判断。
- 板块内条目按交易规模从大到小排列。
- 除机构名/标的名称外，其余文字不要使用"**"或其他markdown符号；项目符号统一使用"• "。
- 同一板块内多条目之间空一行分隔。
